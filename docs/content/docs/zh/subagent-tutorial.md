---
title: "AI Agent 开发实操课: Subagent"
description: "以 AlexNet 论文为例，通过调用现成 Subagent 和实现复现难度 Subagent 两道练习，构建论文复现调查 Agent。"
prev: false
next: false
head:
  - tag: script
  ...
  - tag: style
    content: '.sl-markdown-content blockquote :is(th, td) { min-width: 7rem; } .sl-markdown-content > table :is(th, td) { min-width: 9rem; }'
---

课程仓库：[learn-harness](https://github.com/JackYansongLi/learn-harness)；源码教程：[docs/tutorial.md](https://github.com/JackYansongLi/learn-harness/blob/main/docs/tutorial.md)。
...
你可以用仓库中的[AlexNet 论文 PDF](https://github.com/JackYansongLi/learn-harness/blob/main/examples/alexnet/paper.pdf)测试你写的程序。
...
![Paper2Agent 从论文代码生成工具、测试并供 Agent 调用的流程图](/images/subagent/paper2agent.svg)
