...
描述细胞生长的水动力方程为：
$$
4\eta \frac{dR}{dt} = \frac{2\sigma (p_g - p)}{R} \quad (14.10)
$$
...
$$
\Delta p = \frac{\sigma}{R_1} + \frac{\sigma}{R_2}
$$
...
如果所有这些扰动都倾向于减小，那么流体是稳定的。如果某些扰动倾向于放大，则流体被认为是不稳定的。通常，物理变量 $ V(x, t) $ 可以表示为稳态分量 $ \bar{V}(x) $ 和扰动分量 $ \tilde{V}(x, t) $ 之和：

$$
V(x, t) = \bar{V}(x) + \tilde{V}(x, t). \quad (14.25)
$$

在获得未知量的稳态值后，扰动变量的演化方程中仅保留扰动变量的一次项，并作为时间的函数求解。通常假设扰动具有分离形式：

$$
\tilde{V}(x, t) = \hat{V}_0(x) e^{\sigma t}, \quad (14.26)
$$

其中 $ \hat{V}_0(x) $ 是扰动的初始振幅，而 $ \sigma $ 是复特征值。如果特征值的实部为负，即 $ \text{Re}(\sigma) < 0 $，则扰动将衰减，表明流体是稳定的。如果 $ \text{Re}(\sigma) > 0 $，扰动将随时间增长，表明流体是不稳定的。因此，$ \text{Re}(\sigma) = 0 $ 表示不稳定性的开始。熔体的粘弹性度量是魏斯纳数，定义为：

$$
U_{Wi} = \frac{\lambda}{H}, \quad (14.27)
$$

其中 $ \lambda $ 是流体的特征时间，$ U $ 是特征速度，通常取为熔体的平均速度，而 $ H $ 是特征长度尺度，通常取为注塑模拟中型腔半隙宽。
...
$$
\frac{d\mathbf{r}(i)}{dt} = \frac{P(i)}{m(i)} + \gammȧ \frac{e_1 \cdot \mathbf{r}^3(i)}{3} + \epsiloṅ \mathbf{r}(i), \quad (14.33)
$$

$$
\frac{dP(i)}{dt} = \mathbf{F}(i) - \gammȧ \frac{e_1 \cdot P^3(i)}{3} - \zeta P(i) - \epsiloṅ P(i), \quad (14.34)
$$

$$
\frac{dV}{dt} = 3 \epsiloṅ V, \quad (14.35)
$$

其中，$\mathbf{r}(i)$、$P(i)$ 和 $m(i)$ 分别表示原子 $i$ 的位置、特殊动量和质量；$e_1$ 是流动方向的单位向量；$\gammȧ$ 是剪切率；$\mathbf{F}(i)$ 是系统中其他所有原子对原子 $i$ 的总力；$V$ 是模拟盒的体积，其动力学由膨胀率 $\epsiloṅ$ 管理。在所有三个方向上应用了周期边界条件进行静止模拟。对于平面剪切模拟，应用了 Lees-Edwards 滑动砖周期边界条件 [223]。最后，从微观信息计算了所需的宏观性质，如温度、压力、密度、热膨胀系数、结晶度和应力。该模拟能够监控在受控温度、压力、冷却速率和流动条件下从无定形结构到半结晶结构的形态转变，并有助于揭示剪切或预剪切对结晶速度增强的影响。MD 模拟方法的研究和开发仍在进行中。
...
$$
f(x) \approx \sum_{b=1}^{N} m_b W\left(\frac{|x - x_b|}{h}\right) f(x_b), \quad (14.36)
$$
