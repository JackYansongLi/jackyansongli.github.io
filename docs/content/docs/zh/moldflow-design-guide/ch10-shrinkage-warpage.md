...
压力、体积和温度的关系可用 PVT 关系表示；它将比容，即密度的倒数，与温度和压力关联。聚合物 PVT 数据通常投影到比容和温度平面。通常的注塑压力只能使体积收缩减少约一半。室温常压点、典型成型温度点和典型成型保压点之间的关系显示，若要让熔体到固态的收缩为零，所需压力远高于注塑机可提供的压力，单靠施压消除收缩并不现实。
